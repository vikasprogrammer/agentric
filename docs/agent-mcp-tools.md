...
| `update` | `POST /api/update` | messages | W | non-blocking progress note (session-owner scoped); optional `subject`/`step`/`of` declare the run's POSITION, which the console pairs with a server-derived forward/stuck/circling/blocked verdict (`src/state/session-progress.ts` — the agent supplies the denominator, never the verdict) |
...
| `session_history` | `GET /api/agent/sessions` | `TerminalManager.sessionsForAgent` | R | **episodic self-query** — the agent's OWN past runs ("have I done this before, how did it go?"), the run-history companion to the semantic memory plane. Own-scoped server-side to the caller's agent id (never a sibling's); excludes the current run; metadata only (id/title/status/rating/dates), not transcripts; optional `query` substring over title/task |
| `session_open` | `GET /api/agent/session` | `readConversation` / `summarizeConversation` (`src/edge`) | R | opens ONE of the caller's own past sessions and returns the friendly transcript **timeline** (`readConversation` → user/assistant/activity turns, tail-capped at 60), or with `summary:true` a throwaway-claude **recap** of the whole run. Own-scoped: the target must belong to the caller's agent (`sessionsForAgent` lookup) else **403**. Falls back to the raw `session-<id>.log` pane tail for a headless run with no structured transcript; summary audited `session.summarized` (`principal: agent:<id>`) |
...
| `task_update` | `POST /api/tasks/update` | `TaskStore.update` | W | status/note/reassign/reprioritise/`due` (ISO date, or "" to clear); link/unlink `goalId`, set/clear `criteria`, replace `dependsOn` (`[]` clears); closes a dispatched loop. **`blockedOn`** (`human`\|`agent`\|`external`) declares what a `blocked` task waits on and ROUTES the wake-up: `human` cards the task owner and does NOT wake the delegating agent (it cannot make a decision only a person can), `agent`/`external` wake the caller as before, unstated keeps the old behaviour. Cleared automatically when the task leaves `blocked` |
| `task_say` | `POST /api/tasks/say` | `Automations.postTaskDiscussion` → `TerminalManager.postTaskMessage` | W | posts into a task's **Discussion** (the threaded conversation on a task; `type='task.chat'`, `task` audience — EXCLUDED from the Inbox feed, so it's quiet). Parses `@mentions`: an `@member` → an addressed Inbox card + DM (`mentionMember`); an `@agent:<id>` → resumed/spawned ON the task (`continueTaskThread`, reusing the thread-continuity engine). Read the thread via `task_get`'s `discussion`; audited `task.said`. **Two-way:** a human's plain message in the room is DELIVERED into the live run working the task (typed into its pane, prefixed `[task discussion] <name>:`, or used to answer its pending `ask`); several live runs → the human is asked which one, and nothing is delivered until they pick. See `docs/tasks-plan.md` §9 |
...
| `github_token` | `POST /api/agent/github/credential` | `GithubIdentity.ensureBotToken(…, org)` | W | get a company-bot token for a NAMED ORG — the `gh` half of multi-org (`docs/github-multi-org-plan.md`). A bot token covers ONE installation; plain `git` already resolves the right one per repository through the session's credential helper, but `gh` reads `GH_TOKEN` and ignores credential helpers, so it is stuck on the primary org. Returns the token for the agent to `export GH_TOKEN=…` before a `gh`-only operation (`gh pr create`, `gh issue`, `gh api`) in another org, and says plainly that it REPLACES the ambient token for the rest of the shell. Typed refusals: `not_installed` (the App isn't on that org — not retryable, ask an owner to install it), `member_identity`, `not_configured`. **Only when `GH_ORG_TOKEN=1`** — set at launch when the App spans several orgs AND the run is on the bot lane; deliberately withheld from a run acting as a human who linked their own GitHub (their user token already spans their orgs, and a bot token would re-author their work as the bot) and from single-org tenants. Shares the route + the member guard with the git credential helper. Audited `github.bot_token.minted` (`via:'credential-helper'`) on a real mint only — a cached hour-long token is reused silently |
...
| `telegram_reply` | `POST /api/agent/telegram/reply` | TelegramSocket | W | only when `TELEGRAM_REPLY=1` (chat-triggered); posts back into the bound Telegram chat as a reply, plain text |
| `clickup_reply` | `POST /api/agent/clickup/reply` | ClickupIngress | W | only when `CLICKUP_REPLY=1` (ClickUp-triggered) |
...
| `image_edit` | `POST /api/agent/image/edit` | `TerminalManager.editImage` → `ImageBackend.editImage` + `ArtifactStore.ingest` | W | edit / upscale / preset an EXISTING image (source = a Library artifact id, a working-folder file, or an http(s) URL) → saved as a NEW Library image (source never changed), returns the new id. `prompt` transforms it, `scale` (2/4) upscales, `operation:"remove-background"` returns a transparent PNG. Governed as capability `image.edit` (`amountUsd` estimate, money-cap rule) then the vendor call; audited `image.edited`. Exposed with `IMAGE_GEN=1`, but the edit/upscale backend needs an **Atlas** key |
...
63 always-on tools + 13 conditional (the conditional ones — `answer`, `slack_reply`/`discord_reply`/`telegram_reply`,
`slack_send`/`slack_dm`, `discord_send`/`discord_dm`, `image_generate`/`image_edit`, `video_generate`,
`video_understand`, `github_token` — are exposed only when their env flag is set, i.e. the run is
chat-triggered, the backend/integration is configured, or (for `github_token`) the workspace App spans
several orgs). Read-only tools carry `annotations.readOnlyHint`; `forget`
...
- ~~**Episodic self-query.**~~ **Closed** by `session_history` + `session_open`: an agent lists its own
  past runs (id/title/status/rating, own-scoped) and reopens any one — the friendly transcript timeline
  or a summary — so "have I done this before, how did it go?" is answerable alongside semantic `recall`.
